...
$(function () {
    ...
    // Initialize QueryBuilder with filters
    buildQueryBuilder();

    // Load GeoJSON and initialize features
...
function buildQueryBuilder() {
    $("#query-builder").queryBuilder({
        allow_empty: true,
        filters: properties
            .filter(prop => prop.filter) // Only properties with filters
            .map(prop => ({
                id: `properties.${prop.value}`, // Nested field reference
                label: prop.label,
                type: prop.filter.type,
                operators: prop.filter.operators
            }))
    });
}
...
function applyFilter() {
    ...
    if (!$.isEmptyObject(rules)) {
        ...
        // Apply the filter to GeoJSON using AlaSQL
        const query = `SELECT * FROM ? ${sql ? "WHERE " + sql : ""}`;
        alasql(query, [geojson.features], function (filteredFeatures) {
            // Clear and re-add filtered features to the map
            featureLayer.clearLayers();
            featureLayer.addData(filteredFeatures);
            syncTable(filteredFeatures);
        });
    } else {
        $("#output").text("No valid rules to apply.");
    }
}

// Sync map and table with filtered features
function syncTable(filteredFeatures) {
    tableFeatures = filteredFeatures.map(feature => feature.properties);
    $("#table").bootstrapTable("load", JSON.parse(JSON.stringify(tableFeatures)));
    const featureCount = $("#table").bootstrapTable("getData").length;
    $("#feature-count").text(`${featureCount} visible feature${featureCount === 1 ? "" : "s"}`);
}

// Button Event Listeners for Filter
$("#apply-filter-btn").on("click", applyFilter);
